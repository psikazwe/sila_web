...
class ActionsColumn(Column):
    ...
    def render(self, record):
        ...
        view_details_url = reverse('admin:affiliate-detail', args=[record.id])
        view_details_button = f'<a href="{view_details_url}"><i class="fa-regular fa-eye"></i></a>'
        # Second action button - Edit
        edit_url = reverse('admin:affiliate-update', args=[record.id])  # Adjust the URL pattern as necessary
        edit_button = f'<a href="{edit_url}"><i class="fa-regular fa-pen-to-square"></i></a>'
        # Third action button - Delete
        delete_url = reverse('admin:affiliate-delete', args=[record.id])  # Adjust the URL pattern as necessary
...
class LogoColumn(Column): 
    def render(self, record):
        formatted_logo = format_html(f'<img src="{record.logo.url}" alt="{record.code}" class="logo"/>')
        return mark_safe(formatted_logo)
