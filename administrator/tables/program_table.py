...
class ActionsColumn(Column):
    ...
    def render(self, record):
        ...
        view_details_url = reverse('admin:program-detail', args=[record.id])
        view_details_button = f'<a href="{view_details_url}"><i class="fa-regular fa-eye"></i></a>'
        # Second action button - Edit
        edit_url = reverse('admin:program-update', args=[record.id])  # Adjust the URL pattern as necessary
        edit_button = f'<a href="{edit_url}"><i class="fa-regular fa-pen-to-square"></i></a>'
        # Third action button - Delete
        delete_url = reverse('admin:program-delete', args=[record.id])  # Adjust the URL pattern as necessary
...
class Table(SingleTableMixin,tables.Table):
    ...
    class Meta:
        ...
        sequence = ('id', 'title', 'category')
        exclude = ( 'description', 'created_on', 'cover', 'id', 'body', 'link' )
